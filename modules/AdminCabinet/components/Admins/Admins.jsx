...
const Admins = observer(({ className }) => {
  ...
  const getText = (admin, field) => {
    let text;
    if (field === 'name') text = (admin.name || admin.surname) && `${admin.name} ${admin.surname || ''}`;
    if (field === 'phone') text = admin.phone;
    return text || '___';
  };
      ...
      >
        ...
        {formType === FORM.CREATE &&
          <form className={cl.form} ref={formRef}>
            ...
            <div className={cl.formButtons}>
              <Button startIcon={<SaveIcon />} onClick={createAdmin}>Сохранить</Button>
